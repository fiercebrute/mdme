...
def test_module(base_url, module, session, headers):
    for template in COMMON_PATHS:
        ...
        try:
            ...
            if r.status_code in (200, 403):
                version = None
                if any(path.startswith(p.format(mod=module)) for p in INFO_YML_PATHS) and r.status_code == 200:
...
def scan_modules(base_url, module_list, user_agent, threads):
    ...
    with ThreadPoolExecutor(max_workers=threads) as executor:
        with requests.Session() as session:
            futures = {
                executor.submit(test_module, base_url, mod, session, headers): mod
...
def main():
    ...
    scan_modules(args.url, modules, args.user_agent, args.threads)
